<template>
  <vx-card :title="$t('orderStatusTypes')">
          ...
          <div class="vx-row">    
            ...
            <div class="w-1/5 mt-6" >
              <vs-button
                ...
                color="success"
                @click.prevent="add"
              >{{$t('addOrderStatusType')}}</vs-button>
            </div>        
            <div class="w-1/6" id="dropdown">
              <vs-dropdown vs-custom-content vs-trigger-click>
                ...
                <vs-dropdown-menu>
                  <vs-dropdown-item>
                    <span @click="perPage=15 ,getOrderStatusTypes()">15</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=25,getOrderStatusTypes()">25</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=50,getOrderStatusTypes()">50</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=75,getOrderStatusTypes()">75</span>
                  </vs-dropdown-item>
                </vs-dropdown-menu>
              </vs-dropdown>
            </div>
      ...
      >
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="p-0" :data="data[indextr].orderStatusTypeId">
              <span  @click="update(data[indextr].orderStatusTypeId)">
            ...
            <vs-td class="p-0" :data="data[indextr].orderStatusTypeId">
              <ul class="switch-container">
                <li>
                  <span
                    @click="preventTableClickAction=true, openConfirm(data[indextr].orderStatusTypeId)"
                  >
    ...
    <vs-pagination class="m-4" :total="totalPage" v-model="currentPage" @change="getOrderStatusTypes"></vs-pagination>
  </vx-card>
</template>
<script>
export default {
  ...
  methods: {
    ...
    acceptAlert() {
      this.$http
        .delete("/order-status-type/" + this.orderStatusTypeId, {
        })
        .then(response => {
          this.$vs.notify({
            color: "success",
    ...
    add: function() {
      this.$router.push({ path: "/pages/add-order-status-type" });
    },
    update: function(id) {
      this.$router.push({ path: "/pages/edit-order-status-type/" + id });
    }
  },
  created() {
    this.getOrderStatusTypes()
