        ...
        <div class="vx-row">
          ...
          <div class="vx-col w-1/8 mr-4">
            <vs-button class="mt-10 px-3" color="primary" type="border" icon-pack="feather" icon="icon-credit-card"
              ...
              v-if="isCustomer && (order.paymentStatus !== 'Paid' && order.paymentStatus !== 'PostPaid' && order.paymentStatus !== 'PartialPaid')">
              {{$t('submitPayment')}}
            </vs-button>
          </div>
                ...
                <vs-tr v-for="(orderItem, index) in order.orderItems" :key="index">
                  ...
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType==ordered">{{status.quantity}}</div>
                    </div>
                  </vs-td>
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType=='Ready for Shipment'">{{status.quantity}}
                      </div>
                    </div>
                  </vs-td>
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType=='In Transit'">{{status.quantity}}</div>
                    </div>
                  </vs-td>
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType=='Delivered' ">{{status.quantity}}</div>
                    </div>
                  </vs-td>
                  -->
                </vs-tr>
            ...
            <vs-table hoverFlat stripe :data="order.orderItems" v-if="order.orderType=='Pick Up'">
              ...
              <template slot-scope="{data}">
                <vs-tr v-for="(orderItem, index) in order.orderItems" :key="index">
                  ...
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType==readyForPickUp">{{status.quantity}}
                      </div>
                    </div>
                  </vs-td>
                  <vs-td>
                    <div class="vx-row" v-for="(status, indexes) in orderItem.orderItemStatusInfo" :key="indexes">
                      <div class="vx-col w-1/3" v-if="status.orderStatusType=='Picked Up'">{{status.quantity}}</div>
                    </div>
                  </vs-td>
                  -->
                </vs-tr>
...
export default {
  ...
  methods: {
    ...
    checkIfOrdered(order){
       ...
       for(let i = 0;i<order.orderItems.length;i++){
          for(let j=0;j<order.orderItems[i].orderItemStatusInfo.length;j++){
             if(order.orderItems[i].orderItemStatusInfo[j].orderStatusType==ordered){
                 return true;
             }
         }
       }
     ...
     checkIfReadyForPickUp(order){
       for(let i = 0;i<order.orderItems.length;i++){
         for(let j=0;j<order.orderItems[i].orderItemStatusInfo.length;j++){
             if(order.orderItems[i].orderItemStatusInfo[j].orderStatusType==readyForPickUp){
                 return true;
             }
         }
    ...
    checkIfPickedUp(order){
       for(let i = 0;i<order.orderItems.length;i++){
          for(let j=0;j<order.orderItems[i].orderItemStatusInfo.length;j++){
             if(order.orderItems[i].orderItemStatusInfo[j].orderStatusType=='Picked Up'){
                 return true;
             }
         }
