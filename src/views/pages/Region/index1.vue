        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="p-0" :data="data[indextr].id">
              <span @click="updateRegion(data[indextr].regionId)">
...
export default {
  ...
  methods: {
    ...
    searchRegions() {
      ...
      this.isLoading = true;
      this.$http
        .get("/region?searchKey="+this.searchKey/*, {
    ...
    goToDetail(regionName) {
      if (true/*this.preventTableClickAction*/) return;
      this.$router.push({
        path: "/pages/regions/" + regionName.regionId
      });
    },
    search: function(searchKey) {
      this.$http
        ...
        .then(response => {
          this.regions = response.data;
          this.total = thisIns.regions[0].totalPages;
    ...
    pagination: function() {
      ...
      this.$http
        ...
        .then(function(response) {
          ...
          thisIns.total = thisIns.regions[0].totalPages;
          thisIns.$store.dispatch("user/populateUser", response.data);
          if (thisIns.updateStatus == true) {
            for (let i = 0; i < thisIns.regions.length; i++) {
              if (thisIns.regions[i].regionStatus == "Active")
  ...
  updated() {
    if (this.searchRegion == "" && this.increament != 0) {
      const thisIns = this;
      this.$http
        .get("/region", {
          params: { page: this.currentPage, limit: this.limit }
