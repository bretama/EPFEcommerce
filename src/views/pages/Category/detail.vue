    ...
    <vx-card id="internal-card" no-shadow card-border>
      ...
      >
        <template>
          <vs-tr>
            <vs-td
              class="pr-4 w-1/4">{{$t('inventoryType')}}</vs-td>
            <vs-td class="p-0">{{item.inventoryType}}</vs-td>
          </vs-tr>
        </template>
      </vs-table>
    </vx-card>
...
export default {
  ...
  methods: {
    ...
    openConfirm(status, Category, color) {
      this.status[0] = true;
      if (status == true) {
        this.categoryStatus = "Disabled";
