<template>
  <vx-card :title="$t('All Registered Customers')">
    ...
    <vs-table  class="stripe" :max-items="currentPage" :data="customers">
      <template slot="thead">
        <vs-th>#</vs-th>
        <vs-th class="p-0" sort-key="firstName">{{$t('Name')}}</vs-th>
        <vs-th class="p-0" sort-key="mobilePhone">{{$t('mobilePhone')}}</vs-th>
        <vs-th class="p-0" sort-key="email">{{$t('Email')}}</vs-th>
        <vs-th class="p-0" sort-key="status">{{$t('Status')}}</vs-th>
      </template>
      <template slot-scope="{data}">
        <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            <vs-td 
            >{{indextr+1}}</vs-td> 
                    <router-link v-bind:to="'/pages/company-detail/' +data[indextr].companyId">
                      <vs-td
              :data="data[indextr].firstName"
            >{{data[indextr].firstName}} {{data[indextr].lastName}} {{data[indextr].grandFatherName}}</vs-td>
        </router-link>        
          <vs-td class="p-0" :data="data[indextr].username">{{data[indextr].mobilePhone}}</vs-td>
          <vs-td class="p-0" :data="data[indextr].id">{{data[indextr].email}}</vs-td>
          ...
          <vs-td class="p-0" :data="data[indextr].id">
            <ul class="switch-container">
              <li>
                <span @click="openConfirm(status[indextr],data[indextr].userId,'success'),increament++" >
...
export default {
  ...
  methods: {
       ...
       updateUser: function(id) {
          this.$router.push({ path: "/pages/update-customer/" + id });
       },
      search: function(searchKey) {
        ...
        .then(response => {
          ...
          this.total=thisIns.customers[0].totalPages;
    ...
    pagination: function() {
      ...
      this.$http
        ...
        .then(function(response) {
          ...
          thisIns.customers = response.data;
          thisIns.total = thisIns.customers[0].totalPages;
          if(thisIns.updateStatus==true){
            for(let i=0;i<thisIns.customers.length;i++){
              if(thisIns.customers[i].userStatus=="Active")
       ...
       this.isLoading=true
        this.$http
          ...
          .then(function(response) {
            ...
            thisIns.total = thisIns.customers[0].totalPages;
            for (let i = 0; i < thisIns.customers.length; i++) {
              if (thisIns.customers[i].userStatus == "Active") thisIns.status[i] = true;
              else thisIns.status[i] = false;
            }
