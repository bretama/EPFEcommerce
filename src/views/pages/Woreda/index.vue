<template>
  <vx-card :title="$t('allWoredas')">
          ...
          <div class="vx-row">
            ...
            <div class="w-1/6" id="dropdown">
              <vs-dropdown vs-custom-content vs-trigger-click>
                ...
                <vs-dropdown-menu>
                  <vs-dropdown-item>
                    <span @click="perPage=15 ,getWoredas()">15</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=25,getWoredas()">25</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=50,getWoredas()">50</span>
                  </vs-dropdown-item>
                  <vs-dropdown-item>
                    <span @click="perPage=75,getWoredas()">75</span>
                  </vs-dropdown-item>
                </vs-dropdown-menu>
              </vs-dropdown>
            </div>
    ...
    <vx-card id="internal-card" title no-shadow card-border>
      ...
      >
        <template slot="thead">
          ...
          <vs-th sort-key="woredaName">{{$t('woredaName')}}</vs-th>
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="pr-4 w-1/4" :data="data[indextr].zoneName">{{data[indextr].zoneName}}</vs-td>
            ...
            <vs-td class="p-0" :data="data[indextr].id">
              <span @click="updateWoreda(data[indextr].woredaId)">
            ...
            <vs-td class="p-0" :data="data[indextr].id">
              <ul class="switch-container">
                <li>
                  <span
                    @click="preventTableClickAction=true, openConfirm(status[indextr],data[indextr].woredaId,'success'),increament++,decreament--"
                  >
    ...
    <vs-pagination class="m-4" :total="totalPage" v-model="currentPage" @change="getWoredas"></vs-pagination>
  </vx-card>
</template>
<script>
export default {
  ...
  methods: {
    ...
    searchWoredas() {
      ...
      this.isLoading = true;
      this.$http
        .get("/woreda?searchKey="+this.searchKey/*, {
    ...
    goToDetail(woredaName) {
      if (true/*this.preventTableClickAction*/) return;
      this.$router.push({
        path: "/pages/woredas/" + woredaName.woredaId
      });
    },
    search: function(searchKey) {
      this.$http
        ...
        .then(response => {
          this.woredas = response.data;
          this.total = thisIns.woredas[0].totalPages;
          thisIns.$store.dispatch("user/populateUser", response.data);
        });
    },
    openConfirm(status, woredaId, color) {
      this.status[0] = true;
      if (status == true) {
        this.woredaStatus = "Disabled";
        this.confirm = "Do You Want to Disable?";
      } else {
        this.woredaStatus = "Active";
    ...
    acceptAlert() {
      this.$http
        .delete("/woreda/" + this.woredaId, {
          woredaStatus: this.woredaStatus
        })
        .then(response => {});
      this.$vs.notify({
        color: "success",
        title: "Status",
        text: "woreda Type Deleted"
      });
      this.getWoredas();
    },
    ...
    pagination: function() {
      ...
      this.$http
        ...
        .then(function(response) {
          thisIns.woredas = response.data;
          thisIns.total = thisIns.woredas[0].totalPages;
          thisIns.$store.dispatch("user/populateUser", response.data);
          if (thisIns.updateStatus == true) {
            for (let i = 0; i < thisIns.woredas.length; i++) {
              if (thisIns.woredas[i].woredaStatus == "Active")
    ...
    add: function() {
      this.$router.push({ path: "/pages/add-woreda" });
    },
    updateWoreda: function(id) {
      this.$router.push({ path: "/pages/edit-woredas/" + id });
    }
  },
  created() {
    const thisIns = this;
  ...
  updated() {
    if (this.searchWoreda == "" && this.increament != 0) {
      const thisIns = this;
      this.$http
        .get("/woreda", {
          params: { page: this.currentPage, limit: this.limit }
